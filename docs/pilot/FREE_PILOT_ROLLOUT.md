...
- `order:status` для клиента и мастера содержит `commercialMode: 'FREE_PILOT'`, `calloutPrice: 0`, `serviceFee: 0`, `freePilot: true`;
- при `PAID_MOCK` событие содержит фактические суммы заявки;
- переключение env не меняет payload уже созданной заявки.
...
Проверить смешанную ленту после переключения режима:

- каждая карточка показывает `Бесплатно` или `1 кредит` по собственному `commercialMode`;
- дата отображается из `slotStart`;
- историческая `PAID_MOCK` заявка продолжает требовать кредит;
- `FREE_PILOT` заявка продолжает принимать бесплатный отклик.
...
- отправлять маскированные HTTP- и Socket.IO-представления.
