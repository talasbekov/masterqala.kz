...
**Находка при подготовке плана:** в обеих enum `OrderStatus` и `PlannedOrderStatus` уже присутствует значение `DISPUTE`, заложенное миграциями этапов 2-3 (`20260715072332_stage2_urgent_orders`, `20260716051634_stage3_planned_orders`), но нигде не используемое в коде — судя по всему, зарезервировано заранее под этот этап по мотивам таблиц переходов §5 бизнес-спеки (`ВЫПОЛНЕНА/В_РАБОТЕ/ЗАКРЫТА → СПОР → ЗАКРЫТА`). Решение ниже (отдельная таблица `Dispute`, без смены статуса заявки) принято осознанно и остаётся в силе — `DISPUTE` в обоих enum остаётся неиспользуемым мёртвым значением, той же природы, что и `CANCELLED_BY_MASTER` в `PlannedOrderStatus` (уже отмечено в бэклоге этапа 3). Не убираем значение из enum (миграция удаления enum-значения в Postgres нетривиальна и не даёт практической пользы), просто не используем.

| Область | Решение |
|---|---|
| Статус заявки при споре | **Не вводим** переход на `DISPUTE` в `OrderStatus`/`PlannedOrderStatus` (хотя значение уже есть в enum, см. находку выше) — отдельная таблица `Dispute` со своим `status: OPEN\|RESOLVED`. Заявка остаётся в своём текущем статусе (`ВЫПОЛНЕНА`/`В_РАБОТЕ`/`ЗАКРЫТА`); единственный эффект открытого спора на state machine заявки — блокировка авто-закрытия. Избегает дублирования статусной логики в двух местах и не ломает существующие гейты. |
