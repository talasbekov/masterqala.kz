    ...
    <div className="flex flex-col gap-3 px-5 pb-3.5 pt-1.5">
      ...
      {preview?.available && (
        <div className="rounded-lg border border-border bg-surface p-3.5">
          ...
          <div className="text-xs leading-relaxed text-on-fill">{t('newOrder.step4Note')}</div>
        </div>
      )}
      <div className="flex items-center justify-between rounded-md border border-border bg-surface p-3">
        <span className="text-sm font-extrabold text-ink">{t('newOrder.step4PaymentMethod')}</span>
      </div>
      <p className="text-xs leading-relaxed text-ink-soft">{t('newOrder.step4CancelNote')}</p>
      {error && <p className="text-sm font-semibold text-danger">{error}</p>}
      <div className="mt-auto" />
      <button
        type="button"
